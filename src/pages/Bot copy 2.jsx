...
import { RobotOutlined, PlayCircleOutlined, PauseCircleOutlined, ReloadOutlined, UpOutlined, DownOutlined, ArrowLeftOutlined, EyeInvisibleOutlined, EyeTwoTone, CopyOutlined, PlusOutlined, DeleteOutlined, EyeOutlined } from '@ant-design/icons'
...
const Bot = () => {
  ...
  // 解析记录相关状态
  const [parseRecords, setParseRecords] = useState([]);
  const [parseRecordsLoading, setParseRecordsLoading] = useState(true);
  const [parseRecordsPagination, setParseRecordsPagination] = useState({
    page: 1,
    pageSize: 10,
    total: 0
  });
  const [parseRecordsViewModalVisible, setParseRecordsViewModalVisible] = useState(false);
  const [currentParseRecord, setCurrentParseRecord] = useState(null);
  ...
  const fetchParseRecords = async () => {
    setParseRecordsLoading(true);
    try {
      const response = await api.get('/api/parse-records', {
        params: {
          page: parseRecordsPagination.page,
          pageSize: parseRecordsPagination.pageSize
        }
      });
      if (response.data && response.data.data) {
        setParseRecords(response.data.data);
        setParseRecordsPagination(prev => ({
          ...prev,
    ...
    <div style={{ padding: '20px' }}>
      ...
      >
        ...
        <TabPane tab="解析记录" key="parseRecords">
                  ...
                  },
                  {
                    ...
                    render: (text) => (
                      <span
                        ...
                        onClick={() => copyToClipboard(text, '分享链接')}
                        title="点击复制"
                      >
                        {text}
                  ...
                  },
                  {
                    ...
                    render: (_, record) => (
                      <Button
                        ...
                        icon={<EyeOutlined />}
                        onClick={() => {
                          setCurrentParseRecord(record)
                          setParseRecordsViewModalVisible(true)
                        }}
                      />
                    )
                  }
                ]}
                dataSource={parseRecords}
                rowKey="id"
                pagination={{
                  current: parseRecordsPagination.page,
                  pageSize: parseRecordsPagination.pageSize,
                  total: parseRecordsPagination.total,
          ...
          <Modal
            title="解析记录详情"
            open={parseRecordsViewModalVisible}
            onCancel={() => setParseRecordsViewModalVisible(false)}
            footer={[
              <Button key="close" onClick={() => setParseRecordsViewModalVisible(false)}>
                关闭
              </Button>
            ]}
            width={800}
              ...
              <div style={{ maxHeight: '60vh', overflowY: 'auto' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                  <div>
                    ...
                    <div style={{ padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '4px' }}>
                      <div style={{ marginBottom: '8px' }}>
                        ...
                        <span>{currentParseRecord.id}</span>
                      </div>
                      <div style={{ marginBottom: '8px' }}>
                        <span style={{ color: '#8c8c8c' }}>解析状态：</span>
                        {getParseStatusTag(currentParseRecord.parse_status)}
                      </div>
                      <div style={{ marginBottom: '8px' }}>
                        <span style={{ color: '#8c8c8c' }}>内容类型：</span>
                        {getContentTypeTag(currentParseRecord.content_type)}
                      </div>
                      <div style={{ marginBottom: '8px' }}>
                        <span style={{ color: '#8c8c8c' }}>发送状态：</span>
                        {getSendStatusTag(currentParseRecord.send_status)}
                      </div>
                      <div style={{ marginBottom: '8px' }}>
                        <span style={{ color: '#8c8c8c' }}>聊天ID：</span>
                        <span>{currentParseRecord.chat_id}</span>
                      </div>
                      <div style={{ marginBottom: '8px' }}>
                        <span style={{ color: '#8c8c8c' }}>图片数量：</span>
                        <span>{currentParseRecord.image_count || 0}</span>
                      </div>
                      <div style={{ marginBottom: '8px' }}>
                        <span style={{ color: '#8c8c8c' }}>创建时间：</span>
                        <span>{formatDate(currentParseRecord.created_at)}</span>
                      </div>
                      <div>
                        <span style={{ color: '#8c8c8c' }}>更新时间：</span>
                        <span>{formatDate(currentParseRecord.updated_at)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div style={{ padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '4px', marginBottom: '16px' }}>
                      <p style={{ margin: 0 }}>{currentParseRecord.title || '-'}</p>
                    </div>

                    <h4 style={{ marginBottom: '8px', color: '#8c8c8c' }}>分享链接</h4>
                    <div style={{ padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '4px', marginBottom: '16px' }}>
                      <p style={{ margin: 0, wordBreak: 'break-all', color: '#1890ff', cursor: 'pointer' }} onClick={() => copyToClipboard(currentParseRecord.share_url, '分享链接')} title="点击复制">
                        {currentParseRecord.share_url || '-'}
                      </p>
                    </div>

                    <h4 style={{ marginBottom: '8px', color: '#8c8c8c' }}>视频URL</h4>
                    <div style={{ padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '4px', marginBottom: '16px', maxHeight: '60px', overflow: 'hidden' }}>
                      <p style={{ margin: 0, wordBreak: 'break-all', color: '#1890ff', cursor: 'pointer', display: '-webkit-box', WebkitLineClamp: '2', WebkitBoxOrient: 'vertical', overflow: 'hidden' }} onClick={() => copyToClipboard(currentParseRecord.video_url, '视频URL')} title="点击复制">
                        {currentParseRecord.video_url || '-'}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ marginTop: '16px' }}>
                  ...
                  <div style={{ padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '4px', marginBottom: '16px' }}>
                    <p style={{ margin: 0, wordBreak: 'break-all', color: '#1890ff', cursor: 'pointer' }} onClick={() => copyToClipboard(currentParseRecord.parse_url, '解析接口')} title="点击复制">
                      {currentParseRecord.parse_url || '-'}
                    </p>
                  </div>
                </div>
                ...
                <div style={{ marginTop: '16px' }}>
                  ...
                  <div style={{ padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '4px', marginBottom: '16px' }}>
                    <p style={{ margin: 0 }}>{currentParseRecord.parse_message || '-'}</p>
                  </div>
                </div>
                ...
                <div style={{ marginTop: '16px' }}>
                  ...
                  <div style={{ padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '4px', marginBottom: '16px' }}>
                    <p style={{ margin: 0 }}>{currentParseRecord.send_message || '-'}</p>
                  </div>
                </div>

                {currentParseRecord.image_urls && (
                  <div style={{ marginTop: '16px' }}>
                    <h4 style={{ marginBottom: '8px', color: '#8c8c8c' }}>图片列表</h4>
                    <div style={{ padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '4px', maxHeight: '200px', overflowY: 'auto' }}>
                      {(() => {
                        try {
                          const images = JSON.parse(currentParseRecord.image_urls)
                          return (
                          ...
                          return <p>{currentParseRecord.image_urls}</p>
                        }
                      })()}
                    </div>
                  </div>
                ...
                {currentParseRecord.message_ids && (
                  <div style={{ marginTop: '16px' }}>
                    <h4 style={{ marginBottom: '8px', color: '#8c8c8c' }}>消息ID列表</h4>
                    <div style={{ padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '4px' }}>
                      {(() => {
                        try {
                          const ids = JSON.parse(currentParseRecord.message_ids)
                          return <span>{ids.join(', ')}</span>
                        } catch (e) {
                          return <span>{currentParseRecord.message_ids}</span>
                        }
                      })()}
                    </div>
                  </div>
                ...
                {currentParseRecord.group_send_results && (
                  <div style={{ marginTop: '16px' }}>
                    <h4 style={{ marginBottom: '8px', color: '#8c8c8c' }}>群组发送结果</h4>
                    <div style={{ padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '4px' }}>
                      {(() => {
                        try {
                          const results = JSON.parse(currentParseRecord.group_send_results)
                          return (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                              {results.map((result, index) => {
                                const groupId = result.group_id
                                const groupUsername = result.group_username
                                const isNegative = String(groupId).startsWith('-100')
                                const cleanChatId = isNegative ? String(groupId).replace('-100', '') : String(groupId)
                                    ...
                                    <div>
                                      ...
                                      {(() => {
                                        try {
                                          ...
                                          return messageIds.map((msgId, msgIndex) => {
                                            let url
                                            if (groupUsername) {
                          ...
                          return <span>{currentParseRecord.group_send_results}</span>
                        }
                      })()}
                    </div>
                  </div>
